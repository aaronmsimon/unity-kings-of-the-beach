        ...
        public override void Update() {
            ...
            if (powerMeterIsActive) {
                powerValue.Value += (powerMeterIsIncreasing ?
                    (servePowerFillSpeedMax - servePowerFillSpeedMin) / (skillLevelMax - 1) * (skillLevelMax - player.Skills.Serving) + servePowerFillSpeedMin :
                    -((servePowerDrainSpeedMax - servePowerDrainSpeedMin) / (skillLevelMax - 1) * (skillLevelMax - player.Skills.Serving) + servePowerDrainSpeedMin))
                    * Time.deltaTime;
