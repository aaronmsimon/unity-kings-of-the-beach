    ...
    public abstract class Athlete : MonoBehaviour
    {
        ...
        public void Spike(Vector3 targetPos) {
            if (!feint) {
                ...
                float skillFactor = Mathf.Lerp(2f, 0.2f, (skills.SpikeSkill - 1f) / 9f);
                float penaltyModifier = timingVar * skillFactor;
                spikeSpeedPenalty = timingVar * window;
                Vector2 randomOffset = UnityEngine.Random.insideUnitCircle.normalized * penaltyModifier;
                Vector3 newTargetPos = new Vector3(targetPos.x + randomOffset.x, targetPos.y, targetPos.z + randomOffset.y);
                Debug.Log($"timingVar: {timingVar} x skill factor: {skillFactor} = {penaltyModifier} | target: {targetPos} + {randomOffset} = {newTargetPos}");
